    ...
    namespace gpu {
      ...
      struct BVH2Engine : public KNNEngine {
        ...
        virtual void doQueryGPU(const core::BVH2::Node *nodes,
                                const uint32_t *leafLists, const vec3f *points,
      ...
      struct BVH2EngineBT : public BVH2Engine {
        std::string toString() const override
        { return "knn::gpu::BVH2::BT"; }

        void doQueryGPU(const core::BVH2::Node *nodes,
                        const uint32_t *leafLists, const vec3f *points,
                        const vec3f *queryPos, int numQueries, int k,
                        float maxQueryDistance, KNNResult *results) override;
      };
    }
  }
}
