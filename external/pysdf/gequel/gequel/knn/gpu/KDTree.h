...
namespace gequel {
  ...
  namespace knn {
    namespace gpu {
      ...
      struct KDTree : public KNNEngine {
        ...
        struct PointData : public abstract::PointData {
          PointData(const std::vector<vec3f> &points)
            : bounds(Tree::computeBounds(points)),
              nodes(buildTree(points,bounds))
          {}
          
          static thrust::device_vector<Tree::Node>
          buildTree(const std::vector<vec3f> &points, const box3f &bounds);

          const box3f bounds;
          const thrust::device_vector<Tree::Node> nodes;
        };
    
        std::string toString() const override
        { return "knn::gpu::KDTree"; }
