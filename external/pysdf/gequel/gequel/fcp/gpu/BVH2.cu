...
namespace gequel {
  ...
  namespace fcp {
    namespace gpu {
      ...
      abstract::PointData *BVH2Engine::createModel(const std::vector<vec3f> &vertices)
      {
        return new BVH2Engine::PointData(vertices);
      }

      // Assume the arrays are already in device memory
      void BVH2Engine::findClosestPoint(FCPResult *resultArray, int numQueries,
                                        /*! the point model of N points that
                                           we're querying from */
                                        abstract::PointData *_model,
                                        /*! the M positions for which we're
                                           querying the kNNs */
                                        const vec3f *queries,
        ...
        doQueryGPU(thrust::raw_pointer_cast(&model->d_nodes[0]),
                   ...
                   thrust::raw_pointer_cast(&model->d_points[0]), queries,
      ...
      __global__ void BVH2BTQuery(const core::BVH2::Node *nodes,
        ...
        results[tid].distance = maxQueryDistance;
        results[tid].pointID = -1;
        ...
        while (curItem.nodeID >= 0) {
          ...
          if (node.count == 0) {
            ...
            for (int i=0;i<node.count;i++) {
              ...
              float nodeDist = length(queryPos[tid] - points[pointID]);
              if (nodeDist < results[tid].distance) {
                results[tid].pointID = pointID;
                results[tid].distance = nodeDist;
              }
            }
          }
          ...
          if (!finishIter) {
            if (curItem.nodeID == 0) {
              ...
              if (path != TraversalPath::FROM_SIBLING && curItem.nodeID != 0 &&
                ...
                if (sibling.distance < results[tid].distance) {
                  curItem = sibling;
